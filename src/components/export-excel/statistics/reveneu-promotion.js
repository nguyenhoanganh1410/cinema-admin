...
export function exportExcel(data, start_date, end_date) {
    ...
    const borderRight_BT = { right: { style: 'thin' }, bottom: { style: 'thin' } };
    let row_index = 8;
    let idStaff_curr;
    let total_revenue = 0;
    ...
    worksheet.addRow(['BÁO CÁO TỔNG KẾT CTKM'])
    worksheet.addRow(['Từ ngày: ' + start_date +"         "+' Đến ngày ' + end_date])
    worksheet.addRow([])
    worksheet.addRow(
        ['STT', 'Tên NVBH', 'Ngày', 'Chiết khấu', 'Doanh số trước CK', 'Doanh số sau CK'],
    )
    // set font header
    ...
    worksheet.addRow({
        ...
        revenueBeforeDiscount: data.reduce((sum, item) => sum + item.totalDiscount, 0),
